...
* AJAX - radio btn press starts recorder and or feeds audio element in browser.
* radio_btn_id is actually the name of the radio, it has a minus before the name if it is removed (stop)
* Python returns a dictionary of multiple key: value pairs that are displayed in the page footer.
* Todo Check all fun for correct requestform POST or GET
*/
function ajax_switch_radio(radio_btn_id: string) {
  const xhr = new XMLHttpRequest();
  xhr.open('POST', '/radio_btn_id');  // Python endpoint POST request (we send data)
  ...
  xhr.onload = function () {
    ...
    // set all uniform
    for (let i: number = 0; i < divRadioBtn.length; i++) {
      let div = divRadioBtn[i] as HTMLElement;
      div.style.color = '#000000';
      div.style.backgroundColor = '#f9f9f9ff';
    }
    // Mark recorder active buttons with a colored rim.
    for (const elem of data.recorder) {
      let rec = document.getElementById('div' + elem) as HTMLElement;
      rec.style.color = '#f9f9f9ff';
      rec.style.backgroundColor = '#fd7f7f';
    }
    /* bottom page footer */
    let pMsg = document.getElementById('pMsg') as HTMLElement;
    pMsg.innerHTML = '';
    ...
    if (data.content == "no_response") {
      ...
      let radio = document.getElementById("div" + data.radio_name) as HTMLElement;
      radio.style.backgroundColor = 'black';
      return;
    }
    /* enable sound */
    let audioR = document.getElementById('audioR') as HTMLMediaElement;
    ...
    if (data.radio_name == radio_btn_id) {  // data.radio_name is ajax return value
      ...
      // Set var for interval title scan for current radio in listen (buffer) mode.
...
function ajax_title_get() {
  ...
  xhr.onload = function () {
    ...
    let pTitle = document.getElementById('pTitle') as HTMLElement;
  ...
  xhr.send(JSON.stringify(glob.playingRadio));
}
;
function ajax_get_config_file() {
   ...
   */
  const xhr = new XMLHttpRequest();
  xhr.open('POST', '/get_config_file');  // could be a GET, send nothing

  xhr.onload = function () {
    var data = JSON.parse(xhr.responseText);
    console.log('resp gc ', data);
    let configFileEdit = document.getElementById('configFileEdit') as HTMLTextAreaElement;
    configFileEdit.value = data.get_config_file;
    // write path of file
    let configFileEditPath = document.getElementById('configFileEditPath') as HTMLSpanElement;
...
function ajax_write_config_file() {
  ...
  xhr.onload = function () {
    ...
    let pConfigResponse = document.getElementById('pConfigResponse') as HTMLParagraphElement;
...
function ajax_get_blacklist_file() {
  ...
  xhr.onload = function () {
    ...
    let blacklistFileEdit = document.getElementById('blacklistFileEdit') as HTMLTextAreaElement;
    blacklistFileEdit.value = data.get_blacklist_file;
    // write path of file
    let blacklistFileEditPath = document.getElementById('blacklistFileEditPath') as HTMLSpanElement;
...
function ajax_write_blacklist_file() {
  ...
  xhr.onload = function () {
    ...
    let pBlacklistResponse = document.getElementById('pBlacklistResponse') as HTMLParagraphElement;
...
function ajax_server_shutdown() {
  ...
  xhr.onload = function () {
    ...
    let pMsg = document.getElementById('pMsg') as HTMLParagraphElement;
    let sMsgShutDown = document.getElementById('sMsgShutDown') as HTMLSpanElement;
    pMsg.innerHTML = '';
