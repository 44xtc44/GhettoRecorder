...
window.addEventListener('load', function () {
  ...
  for(let i = 0; i <= draggable.length - 1; i++) {
    touchMoveMobile(draggable[i] as HTMLElement);
    dragMoveMouse(draggable[i] as HTMLElement);
  };
})
  ;
...
class Glob {
   ...
   */
  constructor(
    public playingRadio: boolean = false,
    public waitShutdownIntervalId: number = 0,
    public animationRuns: number = 0,
    public animationRunsMax: number = 1,
    public windowWidth: number = window.innerWidth
    ) {
  }
  numberRange(start: number, end: number) {  // simulate range() of Python
  ...
  updateScreen() {
    ...
    if (this.windowWidth <= 1080) {
      if(divAirOne !== null) divAirOne.style.top = "26em";
      if(divAirOne !== null) divAirOne.style.left = "2.0em";
      if(spanHeaderCenter !== null) spanHeaderCenter.innerHTML = "Ghetto";
    } else {
      if(divAirOne !== null) divAirOne.style.top = "14em";
      if(divAirOne !== null) divAirOne.style.left = "2.0em";
      if(spanHeaderCenter !== null) spanHeaderCenter.innerHTML = "GhettoRecorder";
    }
  }
  toggleAnimation() {
...
class HiddenOnOff {
   ...
   */
  constructor(
    public isSwitchedOn: {[key: string]: boolean} = {
  ...
  update(options: {element: string, set: boolean}) {
  ...
  toggle(options: {element: string}) {
    let elem = document.getElementById(options.element);

    if (!this.isSwitchedOn[options.element]) {
      if(elem !== null) elem.style.display = 'inline-block';  // toggle
...
class AudioContextCreate {
  ...
  constructor(
    ...
    public audioR: HTMLAudioElement | null = null,
    public gainR: HTMLInputElement | null = null,
    public audioSource: any | null = null
    ) {
  }
  init() {
    /**
    ...
    this.audioR = document.getElementById('audioR') as HTMLMediaElement, 
    ...
    this.gainR = document.getElementById('gainR') as HTMLInputElement,
    ...
    if(this.gainR !== null) this.gainR.addEventListener("input",  (e) => {
      /**
       * Gain controller raise/lower input value if slider in action. 
       * Typescript complains gainR.value is a string.
       */
      if(this.gainNode !== null && this.gainR !== null) this.gainNode.gain.value = parseInt(this.gainR.value);
    });
  }
}
